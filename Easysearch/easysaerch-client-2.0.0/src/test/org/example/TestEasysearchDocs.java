...
public class TestEasysearchDocs extends TestBase {
  ...
  public void testInsertGenericDocWithId() throws IOException {
    ...
    IndexResponse response = client.index(
        i -> i.index("products").id((String) product.getId()).document(product));
  ...
  public void testInsertDocWithId() throws IOException {
    ...
    IndexResponse response = client.index(
        i -> i.index("products").id(product.getSku()).document(product));
  ...
  public void testIndexWithJson() throws IOException {
    Reader input = new StringReader(
        "{'id': 'sn10003', 'name': 'television', 'price': 5500.5}".replace('\'', '"'));
  ...
  public void testBulkDocs() throws IOException {
    ...
    for (Product product : products) {
      br.operations(
          op -> op.index(idx -> idx.index("products").id(product.getSku()).document(product)));
    }
  ...
  public void testSearchDocs() throws IOException {
    ...
    SearchResponse<Product> response = client.search(s -> s.index("products").source(sourceConfig)
        .query(q -> q.match(t -> t.field("name").query(searchText))), Product.class);
  ...
  public void testUpdateDocWithId() throws IOException {
    if (client.indices().exists(ex -> ex.index("products")).value()) {
      ...
      UpdateResponse response = client.update(u -> u.index("products").id("bk-1").doc(product),
          Product.class);

      System.out.println("Indexed with version " + response.version());
    } else {
      System.out.println("the index not exists,please create it and test again.");
    }
  }
  ...
  public void testUpsertDocWithId() throws IOException {
    if (client.indices().exists(ex -> ex.index("products")).value()) {
      ...
      UpdateResponse response = client.update(
          u -> u.index("products").id("bk-1").doc(product).upsert(product).docAsUpsert(true),
          Product.class);
  ...
  public void testUpdateDocByquery() throws IOException {
    if (client.indices().exists(ex -> ex.index("products")).value()) {
      UpdateByQueryResponse response = client.updateByQuery(u -> u.index("products").script(
              s -> s.inline(
                  InlineScript.of(i -> i.lang("painless").source("ctx._source.price += 1000"))))
          .query(q -> q.match(m -> m.field("name").query("T-shirt"))));
  ...
  public void testSimpleAggregation() {
    ...
    try {
      response = client.search(b -> b.index("products").size(0).query(query)
              .aggregations("price-histogram", a -> a.histogram(h -> h.field("price").interval(50.0))),
          Void.class);
    } catch (IOException e) {
      throw new RuntimeException(e);
    }

    List<HistogramBucket> buckets = response.aggregations().get("price-histogram").histogram()
        .buckets().array();
  ...
  public void templatedSearch() {
    try {
      client.putScript(r -> r.id("query-script").script(
  ...
  public void buildTermQuery(String value) {
    ...
    try {
      SearchResponse<Product> search = client.search(s -> s.index("products").query(query),
          Product.class);
